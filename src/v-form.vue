...
export default {
	...
	methods:{
        ...
        resize(e){
            ...
            var me=this,el=me.$el,h=e.height;
            ...
            if(el.children[1]){
                ...
                [].forEach.call(el.children, (ee,i)=> {
                    if(i==el.children.length-1&&ee.tagName=='CENTER'){
                        ...
                        h-=(ee.offsetHeight-2*parseInt(style2.padding));
                        //console.log(style2.padding);
                    }else{
                        ell.push(ee);
                    }
                });
                ell.forEach((ee)=> {
                    if(!el2){
                        ...
                        if(ell.length==1){
                            el2=ee;
                        }else if(!ee.classList.contains('v-scrollable')&&!ee.classList.contains('v-resize')){
                            h-=(ee.offsetHeight+2);
                        }else{
                            el2=ee;
                        }
      ...
      //          console.log(el);
            ...
            style2 = window.getComputedStyle(el);
            h=h-2*parseInt(style2.paddingTop);
            //console.log(el);
    ...
    updated() {
        ...
        me.$el.querySelectorAll("select,input[type=date]:not(.__),input[type=text]:not(.__),textarea:not(.__)").forEach((e)=>{
