		...
		<div class="v-form"> 
			...
			<v-fieldset legend="Informaci&oacute;n de contacto">
				...
				<div class="v-form">
					...
					<v-group v-if="editContact" v-model="o.sex" >
...
export default window.ui({
	...
	created(){
		window.logo=require('@/cdn/images/profile.svg');
		var me=this;
	...
	methods:{
		...
		changeImage(url){
			var me=this;
			axios.post('/api/user/change-image',{image:url,people:me.session.idPer}).then((result) => {
		...
		changePassword(){
			...
			if(o.pass&&o.newPass&&o.confirm==o.newPass){
				if(me.validate(me.$refs.changePassword)){
					console.log(o);
					axios.post('/api/login/change-password',{
							"current":o.pass,
							"new":o.newPass,
							"confirm":o.confirm
						}).then((e) => {
